"""Optional: write ``generated/archive-entries.json`` from curated Spriters Resource ZIP rows.

Same rows are merged into ``candidates``; you do not need to run this for the main pipeline."""
...
from soundpack_builder.core.console_progress import print_status
from soundpack_builder.core.hook_events import TARGET_TO_EVENT, target_filenames_in_order

EVENT_ORDER = target_filenames_in_order()
...
DEFAULT_ARCHIVE_MANIFEST = "generated/archive-entries.json"


def write_archive_manifest(cfg: BuilderConfig, *, out_name: str = DEFAULT_ARCHIVE_MANIFEST) -> tuple[str, int]:
    rows = build()
    ...
    payload = {
        "schemaVersion": 2,
        "description": (
            "Regenerated from soundpack_builder.tools.archive_entries — game ZIP paths used by the "
            "candidates pipeline. Curated downloader rows live in download-manifest.json."
        ),
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Write game-archive candidate JSON (Spriters Resource bundles) for reference.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=f"Path relative to manifests dir (default: {DEFAULT_ARCHIVE_MANIFEST}).",
    )
    parser.add_argument(
        "--no-progress",
        action="store_true",
        help="Disable stderr status before JSON result.",
    )
    args = parser.parse_args(argv)
    cfg = build_config_from_args(args)
    out_raw = (args.output or "").strip()
    out_name = out_raw if out_raw else DEFAULT_ARCHIVE_MANIFEST
    if not args.no_progress:
        print_status("Building archive entry manifest...", file=sys.stderr)
    out, count = write_archive_manifest(cfg, out_name=out_name)
    if not args.no_progress:
        print_status(f"Wrote {count} entries.", file=sys.stderr)
