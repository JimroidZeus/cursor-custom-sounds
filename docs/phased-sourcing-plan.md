...
**Machine-readable config:** `manifests/phased-sourcing.json` (phase list, portal URLs, `sourceSiteIds`). **Print plan:** `uv run --project soundpack_builder python -m soundpack_builder.phased_sourcing` (`--phase 2`, `--json`).

**Build manifests for downloading:** `uv run --project soundpack_builder python -m soundpack_builder.phased_candidates` writes `manifests/phased/phase-{1,2,3}-candidates.json` (downloader-ready where possible), `phase-*-review.json` (portals, ZIPs without paths, sound detail pages not fetched), and `phased-downloadable-all.json`. Add `--fetch-sound-pages` to HTTP-fetch detail pages and pick direct media links using `discovery.htmlEmbeddedAudioRegexes` in `phased-sourcing.json`. Sound-page URLs are matched with `discovery.soundPageUrlRegexes` (extend both lists as you add sources). Phase assignments use the minimum phase index that lists each link’s `siteId` in `phased-sourcing.json`. Tier-1 game ZIPs already covered in `tier1_candidates` are not duplicated from the discovery manifest. Add `tier1UniverseAliases` in `phased-sourcing.json` when discovery uses a different universe slug (e.g. `dragon-ball` → `dragon-ball-z`).
